...
export function ExportPanel({ artifact }: { artifact: ResourceArtifact }) {
  ...
  const exportArtifact = async (format: ResourceExportFormat) => {
    ...
    setError(null);
    try {
      await downloadResourceArtifact(artifact, format);
    } catch (caught) {
      setError(
  ...
  return (
    <div aria-label="资源导出" className="flex flex-col items-end gap-1.5">
      <div className="flex flex-wrap justify-end gap-2">
        {RESOURCE_EXPORT_FORMATS[artifact.resourceType].map((format) => (
          <ExportButton
            key={format}
