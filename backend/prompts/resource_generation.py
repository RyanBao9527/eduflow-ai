...
slide_outline 的标题、用途、要点、视觉建议和讲解提示中的所有非结构性知识表达都必须来自 lessonModel.keyConcepts；
lessonObjectives、teachingFlow、activities 和 assessmentPoints 只用于组织教学，不得作为新增知识的依据。
只有学习目标、课堂任务、练习、实践、总结、回顾、步骤、要求和活动等结构性表达可以作为通用教学文字。
...
def build_structured_output_request(
    ...
    resource_rules = (
        ...
        if request.resource_type == "lesson_plan"
            ...
            "练习、实践、总结、回顾、步骤和要求；"
            "标题、用途、视觉建议和讲解提示中的非结构性知识也只能来自 lessonModel.keyConcepts；"
            "不要求每个字段都包含核心知识点，但整份 PPT 必须覆盖 lessonModel.keyConcepts 中的全部核心知识点。"
        )
    )
    retry_rule = (
        "这是格式修复重试。"
